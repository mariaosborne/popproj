...
note: recent changes to the HMD and HFD website have made these methods non-functional.
```{r}
# Fertility
...
To demonstrate, I will use some downloaded data, and use the `pad_fx_vector` method to format the fertility data 
correctly.
```{r}
# ASFRs
my_Fx <- fread("~mariaosborne/censoc/popproj/NORasfrRR.txt")[Year == 1980]$ASFR
# pad the ASFR vector so that it will be the same length as the Lx vector
my_Fx <- pad_fx_vector(my_Fx)
my_Fx

# Lx values
my_Lx <- fread("~mariaosborne/censoc/popproj/norway_flt.txt")$Lx
my_Lx <- (my_Lx)/100000
my_Lx
```
...
- `time`: can be used in place of nsteps. A vector of of years to project for, e.g. 0:10 or -50:50, or 1960:2000.
...
# The projection can also be specified with a vector of years
...
plot(my_projection$time_vec, colSums(my_projection$K_mat))

# plot number of births
plot(my_projection$time_vec, my_projection$births_vec)
...
# dynamic projection
norway_dynamic <- project_leslie1_dynamic(nsteps=49,
                 Lx_mat = Lx_matrix,
                 Fx_mat = Fx_matrix,
                 starting_pop = starting_pop)

# look at population over time
plot(colSums(norway_dynamic$K_mat))
...
million <- 1000000
plot(time_vec, colSums(norway_dynamic$K_mat), ylim = c(1.9*million, 2.65*million),
     ylab = "female population", xlab = "year")
lines(time_vec, real_population$fpop)
...
```{r}
# Lx 
japan_female_lifetable <- fread("~mariaosborne/censoc/popproj/japan_flt.txt") 
# Fx
japan_Fx <- fread("~mariaosborne/censoc/popproj/japan_asfr.txt") 
...
# population
japan_Nx <- fread("~mariaosborne/censoc/popproj/japan_popsize_both.txt")
...
# we will use initial Lx and Fx values from the Norweigian data
...
# notes
# need to fix variable names!! e0_vec vs. e0_vec
...
plot_projection(coale, plot_predicted_peaks = T, 
                plot_generational_slopes = T, generation_t = 50) ### WHAT
```
...
testing
```{r}
test_proj_time <- project_leslie1(time = 2000:2005, nrr_vec = c(1,1,1,2,2,2), 
                             e0_vec = rep(80,6), Lx_zero = my_Lx, Fx_zero = my_Fx)

test_proj_time

plot(test_proj$time_vec, test_proj$nrr_vec)
```
