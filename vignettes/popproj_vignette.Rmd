...
note: recent changes to the HMD and HFD website have made these methods non-functional. to fix.
```{r}
# Fertility
...
To demonstrate, I will use some downloaded data.
The `pad_fx_vector()` method to format the fertility data correctly, so that it has 111 age groups like the mortality data do.
```{r}
# ASFRs
...
# predicted peaks can be innacurate there are multiple local maxima
```
